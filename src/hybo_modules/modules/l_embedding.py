...
class LorentzPositionEncoding(nn.Module):
    def __init__(self, dropout, dim, manifold, max_len=5000, learnable=True):
        ...
        self.dim = dim
        self.point = LorentzLinear(self.manifold, dim, dim)
...
class LorentzEmbeddings(nn.Module):
    ...
    def forward(self, source, step=None, use_position_encoding=True):
        ...
        if use_position_encoding:
            return self.embedding_dropout(self.pe(source, step))
        else:
            return self.embedding_dropout(source)
