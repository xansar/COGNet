...
class HyperCOGNet(nn.Module):
    ...
    def encode(self, diseases, procedures, medications, d_mask_matrix, p_mask_matrix, m_mask_matrix, seq_length,
        ...
        if self.training:
            ...
            diseases_embed = self.diag_embedding(torch.arange(self.DIAG_PAD_TOKEN + 1).to(device))
            pros_embed = self.proc_embedding(torch.arange(self.PROC_PAD_TOKEN + 1).to(device))
            meds_embed = self.med_embedding(torch.arange(self.MED_PAD_TOKEN + 1).to(device))
            # 这里需要区分验证和测试，测试的时候显然不需要每个
            # 这里计算是按照batch来的，一个batch计算一次，但是验证的时候，应该是一个epoch计算一次就可以了
            # 因此需要将这几个embedding，存储下来
            hyper_rep, linear_rep, disease_embed, pro_embed, med_embed, ddi_embedding = self.hgcn(diseases_embed, pros_embed,
                                                                                           meds_embed)
            ...
            # 测试状态下，不需要更新embedding
            disease_embed = self.disease_embed
            pro_embed = self.pro_embed
            med_embed = self.med_embed
